...
test('applies only active-player positions and preserves all other persisted fields', () => {
  ...
  assert.equal(changed.players.find(player => player.name === 'Fwd starter 1')?.position, FantaSoccerRole.Tribune)
  assert.equal(changed.players.find(player => player.name === 'Fwd tribune 1')?.position, FantaSoccerRole.Forward)
  assert.equal(changed.players.find(player => player.name === 'Sold player')?.position, FantaSoccerRole.Tribune)
  assert.equal(changed.players.find(player => player.name === 'Fwd starter 1')?.price, 10)
})
