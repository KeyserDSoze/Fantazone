...
test('platform background workflow schedules calendar, guarded live votes, final votes and recurring platform ingestion', async () => {
  ...
  assert.match(workflow, /steps\.selected_job\.outcome == 'success'/)
  assert.match(workflow, /Live votes were still attempted and successful canonical diffs were committed, but the Serie A calendar refresh failed\./)
