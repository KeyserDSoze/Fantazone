...
test('processes every unconsumed save across kickoff so a replaced pending Action cannot lose the last valid formation', async () => {
  const fixture = await createFixture(realCalendar([[8, '2026-09-05T18:00:00Z'], [9, '2026-09-12T18:00:00Z']]))
  ...
  const result = await snapshotSavedFormations({ ...fixtureRoots(fixture), fallbackBefore: before, now: new Date('2026-09-05T18:05:00Z') })
  ...
  const day8Source = await readJson<{ sourceCommit: string }>(join(fixture.groupRoot, formationSnapshotSourceDocumentPath(BASKET, SEASON, 8, OWNER)))
  const day9Source = await readJson<{ sourceCommit: string }>(join(fixture.groupRoot, formationSnapshotSourceDocumentPath(BASKET, SEASON, 9, OWNER)))
...
test('hydrates normalized season Team references into a full immutable TeamDay snapshot', async () => {
  const fixture = await createFixture(realCalendar([[8, '2026-09-05T18:00:00Z']]))
  const before = git(fixture.groupRoot, 'rev-parse', 'HEAD')
  const seasonPath = seasonTeamDocumentPath(BASKET, SEASON, OWNER)
  const normalized = encodeSeasonTeamDocument(teamWithPlayer())
  await writeJson(join(fixture.groupRoot, seasonPath), normalized)
  git(fixture.groupRoot, 'add', seasonPath)
  commit(fixture.groupRoot, 'save normalized formation', '2026-09-05T17:59:00Z')

  await snapshotSavedFormations({ ...fixtureRoots(fixture), fallbackBefore: before })

  const current = await readJson<SeasonTeamDocument>(join(fixture.groupRoot, seasonPath))
  assert.equal(current.players[0].playerKey, 'mariorossi')
  assert.equal('name' in current.players[0], false)
  assert.equal('team' in current.players[0], false)

  const frozen = await readJson<Team>(join(fixture.groupRoot, dayTeamDocumentPath(BASKET, SEASON, 8, OWNER)))
  assert.deepEqual(frozen.players[0], {
    name: 'Mario Rossi',
...
test('later pre-kickoff saves overwrite the same target snapshot and the newest commit wins', async () => {
  ...
  const result = await snapshotSavedFormations({ ...fixtureRoots(fixture), fallbackBefore: before })

  assert.equal(result.writtenSnapshots, 2)
  assert.equal((await readJson<Team>(join(fixture.groupRoot, dayTeamDocumentPath(BASKET, SEASON, 8, OWNER)))).name, 'Newest')
  const source = await readJson<{ sourceCommit: string }>(join(fixture.groupRoot, formationSnapshotSourceDocumentPath(BASKET, SEASON, 8, OWNER)))
...
test('a save at or after day 38 kickoff updates the cursor but never creates day 39', async () => {
  ...
  const before = git(fixture.groupRoot, 'rev-parse', 'HEAD')
  const commitSha = await commitTeam(fixture.groupRoot, team(), '2026-09-05T18:00:00Z')
  ...
  const cursor = await readJson<{ processedThroughCommit: string }>(join(fixture.groupRoot, FORMATION_SNAPSHOT_CURSOR_PATH))
  assert.equal(cursor.processedThroughCommit, commitSha)
...
function fixtureRoots(fixture: { groupRoot: string; platformRoot: string }) {
  return { groupRepoRoot: fixture.groupRoot, platformRepoRoot: fixture.platformRoot }
}
...
function team(): Team {
  return { name: 'Current formation', owner: OWNER, additionalOwners: [], moneyFromRank: 0, lastUpdate: null, players: [] }
}

function teamWithPlayer(): Team {
  return {
    ...team(),
    players: [{
      name: 'Mario Rossi',
