...
import { IdentityRole, type GroupRaw } from '../../src/domain/src/index'
import {
  GROUP_DOCUMENT_PATH,
...
function raw(role: number = IdentityRole.Participant): GroupRaw {
  return {
    i: 'amici',
    n: 'Amici',
    l: [],
    u: [{ u: 'Ale', e: 'ale@example.com', r: role }],
    b: [],
  }
}
...
test('opens one selected group and composes all repositories around one store', async () => {
  ...
  client.files.set(`KeyserDSoze/Fantazone.Amici/${GROUP_DOCUMENT_PATH}@main`, {
    sha: 'group-1',
...
test('re-reads selected GroupRaw membership when resolving external identity', async () => {
  const client = new FakeContentClient()
  const key = `KeyserDSoze/Fantazone.Amici/${GROUP_DOCUMENT_PATH}@main`
  client.files.set(key, { sha: 'group-1', content: JSON.stringify(raw()) })
  const runtime = await GroupSessionRuntime.open(connection, client)

  client.files.set(key, { sha: 'group-2', content: JSON.stringify(raw(IdentityRole.None)) })
